...
//   BufferSource -> EQ -> Reverb -> deckGain -> masterGain
//
...
export class Deck {
  ...
  constructor(ctx, masterDestination, opts = {}) {
    ...
    if (!this.liteMode) {
      ...
      this.reverb.output.connect(this.deckGain);
    } else {
      this.reverb = null;
      this.eq.output.connect(this.deckGain);
    }
  ...
  desync(intensity) {
    ...
    if (i === 0) {
      ...
      this.baseDetune = 0;
      if (this.source) this.source.detune.setValueAtTime(0, this.ctx.currentTime);
      return;
    }
