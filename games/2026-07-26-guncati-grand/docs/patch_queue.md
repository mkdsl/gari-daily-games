...
- [x] P2 `src/systems/progression.js` + `src/systems/economy.js` — `applyAllocationEffects()` refaktoran da vraća `{seasonMarketingSpent, seasonCrowdCap}` umesto da mutira; progression.js koristi temp state za crowd calcs i dodaje `seasonMarketingSpent` u setState() (done 2026-08-22, commit PENDING)
